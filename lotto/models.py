...
class GuessNumbers(models.Model):
    ...
    def generate(self): # 로또 번호를 자동으로 생성
        ...
        self.save() # GuessNumbers object를 DB에 저장 / self: 클래스 객체변수 하나 -> new_row

    def __str__(self): # Admin page에서 display되는 텍스트에 대한 변경
        return "pk {} : {} - {}".format(self.pk, self.name, self.text) # pk(primary key)는 자동생성됨 -> 자동으로 생성되는 id열
...
# class Cage(Box):
    ...
    #
    #cage_1 = Cage()
    ## print(cage_1)  # <class Cage ~ 123sdfa>
